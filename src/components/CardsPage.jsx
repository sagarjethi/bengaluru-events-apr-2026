...
function fileSlug(period, format, size, idx, total) {
  ...
  return `bengaluru-events-${head}-${size}-${tag}.png`;
}
...
export default function CardsPage() {
  ...
  async function downloadAll() {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `bengaluru-events-${period?.id || 'cards'}.zip`;
      a.click();
  ...
  const shareText = period
    ? `${period.kind === 'week' ? 'Week of ' : ''}${period.short} in Bengaluru — ${filteredEvents.length} tech events. Full list:`
    : 'Bengaluru tech events directory';
  ...
  // ----- SEO -----------------------------------------------------------------
  const pageTitle = 'Download shareable event cards — Bengaluru Events';
  const pageDesc = 'Generate clean, brand-consistent cards for any week or month of Bengaluru tech events. Carousel slides or a single tall card. Download as PNG and post to X, LinkedIn, or Instagram in one click.';
  const breadcrumbLd = {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:image:alt" content="Bengaluru Events — shareable card studio" />
